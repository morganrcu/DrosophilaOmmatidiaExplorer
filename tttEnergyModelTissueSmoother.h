...
//		drawPrimal(x0,renderer);
		...
		std::for_each(x0->GetAJGraph()->VerticesBegin(),x0->GetAJGraph()->VerticesEnd(),[&](const typename TTissue::AJGraphType::AJVertexHandler & vertexHandler){
			std::for_each(x0->GetAJGraph()->BeginOutEdges(vertexHandler),x0->GetAJGraph()->EndOutEdges(vertexHandler),[&](const  TTissue::AJGraphType::AJEdgeHandler & edgeHandler){
		...
		while(it<MaxIt){
			...
			std::for_each(x0->GetAJGraph()->VerticesBegin(),x0->GetAJGraph()->VerticesEnd(),[&](const typename TTissue::AJGraphType::AJVertexHandler & vertexHandler){
				...
				typedef PlatenessImageType::IndexType IndexType;
				IndexType index;
