...
template<class TTissue> typename TTissue::Pointer cloneTissueDescriptor(const typename TTissue::Pointer & inputTissue){
	...
	for(auto edgeIterator= inputTissue->GetAJGraph()->EdgesBegin();edgeIterator!=inputTissue->GetAJGraph()->EdgesEnd();++edgeIterator++){
		...
		resultAJgraph->AddAJEdge(inputTissue->GetAJGraph()->GetAJEdgeSource(edgeHandler),inputTissue->GetAJGraph()->GetAJEdgeTarget(edgeHandler));
	}
...
template<class TIterator, class TAJGraph> std::tuple<itk::Vector<double, 3>,itk::Point<double,3> > embeddingPlane(const TIterator & beginVertices, const TIterator & endVertices,const typename TAJGraph::Pointer & ajGraph){
	...
	return std::make_tuple(normal,meanPoint);
...
public:
	...
	void DoADMM(){
		...
		double m_LambdaP=pow(2,-5);
		double m_LambdaL=pow(2,-50);
		...
		while(it<MaxIt){
			...
			std::for_each(x0->GetAJGraph()->VerticesBegin(),x0->GetAJGraph()->VerticesEnd(),[&](const typename TTissue::AJGraphType::AJVertexHandler & vertexHandler){
				if(x0->GetAJGraph()->AJVertexDegree(vertexHandler)>=3){
					...
					double shrinked = distPP-m_LambdaP;
					if(shrinked<=0){

						zVertexPlane[vertexHandler]=projectedPosition;

					}else{
						auto projection = position -m_LambdaP*normal;
